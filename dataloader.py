...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.visual_concepts = json.load(open(self.opt.input_concepts_json, 'r'))
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            tmp_concept = np.array(self.visual_concepts[ix],dtype = np.float)
            concept_batch.append(tmp_concept)
        ...
        fc_batch, att_batch, label_batch, gts, infos, concept_batch = \
            zip(*sorted(zip(fc_batch, att_batch, label_batch, gts, infos, concept_batch), key=lambda x: 0, reverse=True))
        data = {}
        data['fc_feats'] = np.stack(sum([[_]*seq_per_img for _ in fc_batch], []))
        data['visual_concepts'] = np.stack(sum([[_]*seq_per_img for _ in concept_batch],[]))
        # merge att_feats
